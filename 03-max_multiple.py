...
def find_greatest_product(number_string, adjacent_digits=13):
    ...
    for i in range(len(number_string) - adjacent_digits + 1): 
        slicing = number_string[i:i + adjacent_digits] #slicing number string to get number + adjacent digits 
        product = 1 #setting it equal to one so it doesn't affect multiplication 
        for character in slicing:
            product *= int(character) #looping through each number and calculating the product of the integers 

        if product > max_product:
            max_product = product #determining the maxium product 
